...
export const saveVideo = (item: VideoItem) => setDoc(doc(db, 'videos', item.id), { id: item.id, title: item.title, youtubeId: item.youtubeId, youtubeUrl: item.youtubeUrl ?? '', category: item.category ?? '', description: item.description ?? '', date: item.date ?? '', active: item.active !== false, order: item.order ?? 0 });
