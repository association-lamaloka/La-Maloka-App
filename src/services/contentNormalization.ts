...
export function normalizeTerms(value: unknown): MembershipTerms { const remote = object(value); const sections = array(remote.sections).map(orderedObject).filter((item) => typeof item.id === 'string' && typeof item.title === 'string' && typeof item.content === 'string') as MembershipTerms['sections']; return { ...DEFAULT_MEMBERSHIP_TERMS, ...remote, sections: sections.length ? sorted(sections) : DEFAULT_MEMBERSHIP_TERMS.sections.map((item) => ({ ...item })) } as MembershipTerms; }

export function normalizeNavigation(value: unknown): NavigationItem[] { const items = array(object(value).items).map(orderedObject).filter((item) => typeof item.id === 'string' && typeof item.label === 'string' && ['accueil','cours','agenda','galerie'].includes(String(item.destination)) && typeof item.active === 'boolean') as unknown as NavigationItem[]; return items.length ? sorted(items) : DEFAULT_NAVIGATION.map((item) => ({ ...item })); }
function normalizeCollection<T extends { id: string; order?: number }>(value: unknown, defaults: T[], valid: (item: Record<string, unknown>) => boolean): T[] { const items = array(value).map(orderedObject).filter(valid) as unknown as T[]; return items.length ? sorted(items) : defaults.map((item,index) => ({ ...item, order: item.order ?? index })); }
