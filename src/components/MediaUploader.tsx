...
export function MediaUploader({ label, currentUrl, onUploaded }: { label: string; currentUrl?: string; onUploaded: (media: UploadedMedia) => void }) {
  ...
  useEffect(() => { mounted.current = true; return () => { mounted.current = false; generation.current++; if (previewUrl.current) URL.revokeObjectURL(previewUrl.current); }; }, []);
  const upload = async (file?: File) => {
    if (!file || activeUpload.current) return;
    ...
    activeUpload.current = true; setUploading(true);
    ...
    try {
      ...
      const blob = await uploadToBlob(pathname, file, {
        ...
        clientPayload: token,
        onUploadProgress: ({ percentage }) => { if (mounted.current && attempt === generation.current) setProgress(Math.round(percentage)); },
      });
    ...
    } catch (error) { if (!mounted.current || attempt !== generation.current) return; setPreview(currentUrl || ''); setProgress(0); setStatus(error instanceof Error && error.message !== 'UPLOAD_FAILED' ? error.message : 'Vercel Blob n’est pas configuré ou la mise en ligne a échoué. Configurez BLOB_READ_WRITE_TOKEN pour cette Preview.'); }
    finally { URL.revokeObjectURL(localPreview); if (attempt === generation.current) { previewUrl.current = null; activeUpload.current = false; if (mounted.current) setUploading(false); } }
  };
  const root = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const form = root.current?.closest('form');
    const preventEarlySave = (event: Event) => { if (activeUpload.current) { event.preventDefault(); event.stopImmediatePropagation(); setStatus('Attendez la fin de la mise en ligne avant d’enregistrer.'); } };
    const reset = () => { generation.current++; activeUpload.current = false; setUploading(false); setProgress(0); setStatus(''); if (previewUrl.current) URL.revokeObjectURL(previewUrl.current); previewUrl.current = null; };
    form?.addEventListener('submit', preventEarlySave, true);
    form?.addEventListener('reset', reset);
    return () => { form?.removeEventListener('submit', preventEarlySave, true); form?.removeEventListener('reset', reset); };
  }, []);
  const choose = (event: ChangeEvent<HTMLInputElement>) => { void upload(event.target.files?.[0]); event.target.value = ''; };
  return <div ref={root} className="rounded-xl border p-3"><label className="text-sm font-bold">{label}<span className="mt-2 flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-zinc-900 p-3 text-white dark:bg-white dark:text-zinc-900"><Upload size={17} /> Choisir une image<input className="sr-only" type="file" disabled={uploading} accept="image/jpeg,image/png,image/webp,image/avif" onChange={choose} /></span></label><p className="mt-2 text-xs text-zinc-500">JPEG, PNG, WebP ou AVIF · 5 Mo maximum</p>{preview && <StructuralImage src={preview} alt="Aperçu de l’image sélectionnée" className="mt-3 h-32 w-full rounded-xl object-cover" />}{progress > 0 && <div className="mt-2 flex items-center gap-2"><progress className="w-full" value={progress} max="100">{progress}%</progress><span className="text-xs">{progress}%</span></div>}{status && <p role="status" className="mt-2 text-xs">{status}</p>}</div>;
}
