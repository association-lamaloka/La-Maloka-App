  ...
  const upload = async (file?: File) => {
    ...
    try {
      ...
      const result = await new Promise<UploadedMedia>((resolve, reject) => {
        const xhr = new XMLHttpRequest(); xhr.open('POST', '/api/media/upload'); xhr.timeout = UPLOAD_TIMEOUT_MS; xhr.setRequestHeader('Authorization', `Bearer ${token}`); xhr.setRequestHeader('Content-Type', file.type);
        xhr.upload.onprogress = (event) => event.lengthComputable && setProgress(Math.round(event.loaded / event.total * 100));
        xhr.onload = () => { let data: Partial<UploadedMedia> & { error?: string } = {}; try { data = JSON.parse(xhr.responseText || '{}'); } catch { reject(new Error('INVALID_SERVER_RESPONSE')); return; } xhr.status >= 200 && xhr.status < 300 && data.url ? resolve(data as UploadedMedia) : reject(new Error(data.error || 'UPLOAD_FAILED')); };
    ...
    } catch (error) { const code = error instanceof Error ? error.message : 'UPLOAD_FAILED'; setPreview(currentUrl || ''); setProgress(0); setStatus(code === 'UPLOAD_TIMEOUT' ? 'Le serveur ne répond pas. La mise en ligne a été annulée après 60 secondes. Vous pouvez réessayer.' : code === 'UPLOAD_CANCELLED' ? 'Mise en ligne annulée. L’image précédente a été conservée.' : code === 'AUTH_REQUIRED' ? 'Votre session a expiré. Reconnectez-vous puis réessayez.' : 'La mise en ligne a échoué. L’image précédente a été conservée ; vous pouvez réessayer.'); }
