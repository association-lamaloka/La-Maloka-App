...
export const driveImageSourceUrl = (fileId: string) => `https://lh3.googleusercontent.com/d/${encodeURIComponent(fileId)}=w2000`;
export default async function handler(request: IncomingMessage, response: ServerResponse) {
  ...
  try {
    const check = await fetch(driveImageSourceUrl(fileId), { redirect: 'follow' });
